...
void PieceMap::init(const Variant* v) {
  ...
  add(MUSKETEER_CANNON, from_betza("llNrrNDK", "musketeerCannon"));
  add(LEOPARD, from_betza("NB2", "leopard"));
  add(SPIDER, from_betza("B2DN", "spider"));
  add(UNICORN, from_betza("CN", "unicorn"));
  add(HAWK, from_betza("DHAG", "hawk"));
  add(MUSKETEER_ELEPHANT, from_betza("KDA", "musketeerElephant"));
  add(FORTRESS, from_betza("B3DfNbN", "fortress"));
